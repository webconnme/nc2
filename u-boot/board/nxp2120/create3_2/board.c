...
#if 1//defined(CREATE_3_2INCH_LCD)
// Edit Patrick
// MOSI
#define SPI_MOSI_HIGH   NX_GPIO_SetOutputValue (1,5,CTRUE)
#define SPI_MOSI_LOW    NX_GPIO_SetOutputValue (1,5,CFALSE)
#define SPI_MOSI_OUTPUT NX_GPIO_SetOutputEnable(1,5,CTRUE)

#define SPI_MOSI_IN  	 NX_GPIO_GetInputValue(1,5) 

// nCS
#define SPI_nCS_HIGH    NX_GPIO_SetOutputValue (2,7,CTRUE)
#define SPI_nCS_LOW     NX_GPIO_SetOutputValue (2,7,CFALSE)
#define SPI_nCS_OUTPUT  NX_GPIO_SetOutputEnable(2,7,CTRUE)
// CLK
#define SPI_CLK_HIGH    NX_GPIO_SetOutputValue (1,4,CTRUE)
#define SPI_CLK_LOW     NX_GPIO_SetOutputValue (1,4,CFALSE)
#define SPI_CLK_OUTPUT  NX_GPIO_SetOutputEnable(1,4,CTRUE)

#define SPI_RESET_HIGH   NX_GPIO_SetOutputValue (2,17,CTRUE)
#define SPI_RESET_LOW    NX_GPIO_SetOutputValue (2,17,CFALSE)
#define SPI_RESET_OUTPUT NX_GPIO_SetOutputEnable(2,17,CTRUE)
#endif

#if 1//defined(CONFIG_MACH_SPICA_PB_DTV)

static void SPI_SetPad (void);
static void SPI_Delay  (void);
static void SPI_Start  (void);
static void SPI_Stop   (void);
static void Write_reg(U8 data);
static void Set_reg(U8 addr);

static void SPI_PutData(U8 addr,U8 data);

static void Ext_LCD_Set(void);
#endif
...
static int check_boot(void)
{
	...
	if (ret < 0) {
		/* this is fatal error */
		printf("[FATAL ERROR] mmc part 0\n");
		return -2;
	}

	ret = run_command("fatls mmc 0:1 upgrade", 0);
	if (ret < 0) {
		printf("%s: fatls upgrade error\n", __func__);
		return -1;
	}
...
static void boot_to_upgrade(void)
{
	int ret;

	ret = run_command("fatload mmc 0:1 81000000 boot/uImage", 0);
	if (ret < 0) {
		printf("%s: fatload boot/uImage error\n", __func__);
		return;
	}

	ret = run_command("fatload mmc 0:1 88000000 boot/rootfs.img.gz", 0);
	if (ret < 0) {
...
int board_init(void)
{
	DBGOUT("%s : start  board initialize ...\n", CFG_SYS_BOARD_NAME);
	init_gpio_pad();
	init_alive_pad();
	init_bus_pad();
#ifdef CONFIG_CHECK_POWERKEY
...
int board_late_init(void)
{
	...
	run_command("mmc part 0", 0);//bok test
	load_boot_logo();
#endif
...
static void init_display(int dev)
{
			 ...
			 ((U32)NX_DPC_FORMAT_MRGB565 == DPCFormat))
	{
		...
		GDither           = NX_DPC_DITHER_6BIT;
			 ...
			 ((U32)NX_DPC_FORMAT_RGB666B == DPCFormat) || 
...
static void SPI_SetPad(void)
{
    ...
    {
       // NX_GPIO_SetPadFunction(2, Bit, NX_GPIO_PADFUNC_GPIO);
...
{
	//GPIOB_4
	NX_GPIO_SetPadFunction (1, 4, NX_GPIO_PADFUNC_1);
	NX_GPIO_SetOutputEnable(1, 4, CFALSE);
	//GPIOB_5
	NX_GPIO_SetPadFunction (1, 5, NX_GPIO_PADFUNC_1);
	NX_GPIO_SetOutputEnable(1, 5,  CFALSE);
}
#endif
static void SPI_Delay(void)
{
    volatile int loop = 0x100;
...
static void SPI_ReadData(U8 addr)
{
	Set_reg(addr);
	SPI_Delay();
	Read_reg();
...
DBGOUT("CREATE 3.2inch LCD INITIALIZE\n");
#if 1
//RESET();
	SPI_RESET_LOW;
	SPI_RESET_OUTPUT;
	...
	SPI_PutData(0xE2,0x15);
	SPI_PutData(0xE5,0x00); 
	SPI_PutData(0xE7,0x00); 
	SPI_PutData(0xE8,0x64); 
	SPI_PutData(0xEC,0x08); 
	SPI_PutData(0xED,0x47);
       SPI_PutData(0xEE,0x20); 
	SPI_PutData(0xEF,0x59);
	// Power on Setting
	SPI_PutData(0x23,0x83);   
	SPI_PutData(0x24,0x79); 
	SPI_PutData(0x25,0x4f);
SPI_PutData(0x29,0x00);  
SPI_PutData(0x2b,0x03);
	SPI_PutData(0x1B,0x1E); 
	// Power on sequence 
	SPI_PutData(0x19,0x01);//
	SPI_PutData (0x31,0x03);
	mdelay(5); 
	SPI_PutData(0x32,0x01);//EPL set 1 
	mdelay(5);
	SPI_PutData(0x1F,0x8C);  
	SPI_PutData(0x1F,0x84);  
	mdelay(10);
	SPI_PutData(0x1F,0x94);  
	mdelay(10);
	SPI_PutData(0x1F,0xD4); 
	mdelay(5); 
	// Gamma Setting 
	SPI_PutData(0x40,0x00); 
	SPI_PutData(0x41,0x25);   
	SPI_PutData(0x42,0x24);   
	SPI_PutData(0x43,0x3c);   
	SPI_PutData(0x44,0x3a);   
	SPI_PutData(0x45,0x3F);   
	SPI_PutData(0x46,0x1d);   
	SPI_PutData(0x47,0x71);   
	SPI_PutData(0x48,0x07);   
	SPI_PutData(0x49,0x04);   
	SPI_PutData(0x4A,0x05);   
	SPI_PutData(0x4B,0x09);   
	SPI_PutData(0x4C,0x14);   
	                              
	SPI_PutData(0x51,0x00);
	SPI_PutData(0x52,0x03);
	SPI_PutData(0x53,0x1b);
	SPI_PutData(0x54,0x1a);
	SPI_PutData(0x55,0x3f);
	SPI_PutData(0x56,0x0e);
	SPI_PutData(0x57,0x62);
	SPI_PutData(0x58,0x0b);
	SPI_PutData(0x59,0x16);
	SPI_PutData(0x5A,0x1a);
	SPI_PutData(0x5B,0x1b);
	SPI_PutData(0x5C,0x18);
	SPI_PutData(0x5D,0xFF);  
	// Display ON Setting   
	SPI_PutData (0x16,0x43);//03 try sec time   
	SPI_PutData (0x28,0x20);   
	mdelay(40); 
	SPI_PutData (0x28,0x38);   
	mdelay(40);                      // Waiting 2 frames al least
	SPI_PutData (0x28,0x3C);  
#endif	
#if 0 // old setting//(CREATE_3_2INCH_LCD)
//RESET();
	SPI_RESET_LOW;
	SPI_RESET_OUTPUT;
	mdelay(60);
	...
	// Register setting for EQ setting 
	SPI_PutData(0xE2,0x15);
	SPI_PutData(0xE5,0x38); 
	SPI_PutData(0xE7,0x38); 
	SPI_PutData(0xE8,0x50); 
	SPI_PutData(0xEC,0x09); 
	SPI_PutData(0xED,0x07); 
	SPI_PutData(0xEF,0x48);
	// Power on Setting
	SPI_PutData(0x23,0x72);   
	SPI_PutData(0x24,0x57); 
	SPI_PutData(0x25,0x71);  
	SPI_PutData(0x1B,0x1E); 
	SPI_PutData(0x1A,0x01);
	// Power on sequence 
	//SPI_PutData(0x19,0x01);//02->03   //RGB I/F  
	SPI_PutData (0x31,0x03);
	mdelay(5); 
	SPI_PutData(0x32,0x01);//EPL set 1 
	mdelay(5);
	...
	// Gamma Setting 
	//Test
	SPI_PutData(0x01,0x00); 
	mdelay(5); 
	//Test
	SPI_PutData(0x40,0x08); 
	SPI_PutData(0x41,0x2F);   
	SPI_PutData(0x42,0x2D);   
	SPI_PutData(0x43,0x3E);   
	SPI_PutData(0x44,0x3D);   
	SPI_PutData(0x45,0x3F);   
	SPI_PutData(0x46,0x2B);   
	SPI_PutData(0x47,0x7A);   
	SPI_PutData(0x48,0x06);   
	SPI_PutData(0x49,0x05);   
	SPI_PutData(0x4A,0x07);   
	SPI_PutData(0x4B,0x0E);   
	SPI_PutData(0x4C,0x15);   
	                              
	SPI_PutData(0x51,0x02);
	SPI_PutData(0x52,0x01);
	SPI_PutData(0x53,0x12);
	SPI_PutData(0x54,0x10);
	SPI_PutData(0x55,0x37);
	SPI_PutData(0x56,0x05);
	SPI_PutData(0x57,0x54);
	SPI_PutData(0x58,0x0A);
	SPI_PutData(0x59,0x11);
	SPI_PutData(0x5A,0x18);
	SPI_PutData(0x5B,0x1A);
	SPI_PutData(0x5C,0x19);
	SPI_PutData(0x5D,0xFF);  
	// Display ON Setting
	//SPI_PutData (0x31,0x02);
	//SPI_PutData (0x16,0x09);    
	SPI_PutData (0x16,0x4B);    
	SPI_PutData (0x28,0x20);   
	mdelay(40); 
	SPI_PutData (0x28,0x38);   
	mdelay(40);                      // Waiting 2 frames al least
