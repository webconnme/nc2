...
void board_preboot_os(void)
{
	...
	for (i = 0; i < 4; i++) {
		for (det_mod = 0; 6 > det_mod; det_mod++) {
			NX_ALIVE_SetDetectMode(det_mod, i, CFALSE);
		}
...
#endif
//fuqiang
        int grp = (CFG_PIO_LCD_POWER_EN >> 5);
        int bit = (CFG_PIO_LCD_POWER_EN & 0x1F);
        ...
        grp = (CFG_PIO_LCD_DISP_EN >> 5);
        bit = (CFG_PIO_LCD_DISP_EN & 0x1F);
        NX_GPIO_SetPadFunction (grp, bit, NX_GPIO_PADFUNC_GPIO);
        NX_GPIO_SetOutputEnable(grp, bit, CTRUE);
        NX_GPIO_SetOutputValue (grp, bit, CFALSE);

        grp = (CFG_PIO_LCD_BACKLIGHT_EN >> 5);
        bit = (CFG_PIO_LCD_BACKLIGHT_EN & 0x1F);
        NX_GPIO_SetPadFunction (grp, bit, NX_GPIO_PADFUNC_GPIO);
        NX_GPIO_SetOutputEnable(grp, bit, CTRUE);
        NX_GPIO_SetOutputValue (grp, bit, CFALSE);
	mdelay(200);
...
#endif
//fuqiang
	grp = (CFG_PIO_LCD_POWER_EN >> 5);
	bit = (CFG_PIO_LCD_POWER_EN & 0x1F);
	NX_GPIO_SetPadFunction (grp, bit, NX_GPIO_PADFUNC_GPIO);
	NX_GPIO_SetOutputEnable(grp, bit, CTRUE);
	//NX_GPIO_SetOutputValue (grp, bit, CFALSE);
	//mdelay(200);
...
#else
        U32 pwrsrc[] = {
                 CFG_PWR_POWER_CTL_ALIVE1,
                 };

        U32 detmod[] = {
                PWR_DECT_SYNC_HIGHLEVEL, };
