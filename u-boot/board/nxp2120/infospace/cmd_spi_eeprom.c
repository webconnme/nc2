...
int do_update(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	...
	if (strcmp(cmd, "1stboot") == 0) {
		...
		maxsize = CONFIG_RAMDISK_SIZE;
		offset  = CONFIG_RAMDISK_OFFSET;

		printf(" eeprom update ramdisk 0x%08x, mem 0x%08x, size %d\n", offset, addr, size);
	}
	else if (strcmp(cmd, "kernel") == 0) {
		maxsize = CONFIG_KERNEL_SIZE;
		offset  = CONFIG_KERNEL_OFFSET;

		printf(" eeprom update uImage 0x%08x, mem 0x%08x, size %d\n", offset, addr, size);
	}else {
		cmd_usage(cmdtp);
		return 1;
	}
...
U_BOOT_CMD(
	...
	"update eeprom data",
	" ramdisk addr cnt (hex)\n"
	"    - update ramdisk loder (cnt max 5.7 Mbyte)\n"
	" kernel addr cnt (hex)\n"
	"    - update kernel loder (cnt max 2 Mbyte)\n"
	" uboot addr cnt (hex)\n"
