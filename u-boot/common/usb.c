...
// psw0523 add for CREATE Hub
#define HUB_CLEAR_TT_BUFFER		8
int usb_hub_clear_tt_buffer(struct usb_device *dev, unsigned short devinfo, unsigned short tt)
{
	int ret;
	USB_HUB_PRINTF("%s--> devinfo(0x%x), tt(0x%x)\n", __func__, devinfo, tt);
	ret = usb_control_msg(dev, usb_sndctrlpipe(dev, 0),
			HUB_CLEAR_TT_BUFFER, USB_RT_PORT, devinfo,
			tt, NULL, 0, 1000);
	USB_HUB_PRINTF("ret: %d\n", ret);
	return ret;
}
