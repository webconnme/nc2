...
static int part_validate(struct mtdids *id, struct part_info *part)
{
	...
	if (part->offset > id->size) {
		printf("%s: offset 0x%08x%08x beyond flash size 0x%08x%08x\n",
				id->mtd_id, (uint)(part->offset>>32), (uint)(part->offset&-1),
				(uint)(id->size>>32), (uint)(id->size&-1));
		return 1;
	}
...
static int part_parse(const char *const partdef, const char **ret, struct part_info **retpart)
{
	...
	*ret = NULL;
	...
	debug("+ partition: name %-22s size 0x%08x%08x offset 0x%08x%08x mask flags %d\n",
			part->name, (uint)(part->size>>32), (uint)(part->size&-1),
			(uint)(part->offset>>32), (uint)(part->offset&-1), part->mask_flags);
...
static void list_partitions(void)
{
	...
	list_for_each(dentry, &devices) {
		...
		list_for_each(pentry, &dev->parts) {
			part = list_entry(pentry, struct part_info, link);
			printf("%2d: %-20s0x%08x%08x\t0x%08x%08x\t%d\n",
					part_num, part->name, (uint)(part->size>>32), (uint)(part->size&-1),
					(uint)(part->offset>>32), (uint)(part->offset&-1), part->mask_flags);
	...
	if (current_mtd_dev) {
		part = mtd_part_info(current_mtd_dev, current_mtd_partnum);
		if (part) {
			printf("\nactive partition: %s%d,%d - (%s) 0x%08x%08x @ 0x%08x%08x\n",
					MTD_DEV_TYPE(current_mtd_dev->id->type),
					current_mtd_dev->id->num, current_mtd_partnum,
					part->name, (uint)(part->size>>32), (uint)(part->size&-1),
					(uint)(part->offset>>32), (uint)(part->offset&-1));
		} else {
...
static int delete_partition(const char *id)
{
	...
	if (find_dev_and_part(id, &dev, &pnum, &part) == 0) {
		...
		debug("delete_partition: device = %s%d, partition %d = (%s) 0x%08x%08x@0x%08x%08x\n",
...
static int parse_mtdids(const char *const ids)
{
	...
	while(p && (*p != '\0')) {
		...
		debug("+ id %s%d\t0x%08x%08x bytes\t%s\n",
				MTD_DEV_TYPE(id->type), id->num,
				(uint)(id->size>>32), (uint)(id->size&-1), id->mtd_id);
