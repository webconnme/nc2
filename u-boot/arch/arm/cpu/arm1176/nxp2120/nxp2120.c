...
#define PHYTERMCTRL		*(volatile unsigned long *)(0xc00148B4)
...
void mem_cfg_info(void)
{
	...
	switch(CPUODT) {
	case 0:		printf("No Odt"); 	break;
	case 1:		printf("150 ohm"); 	break;
	case 2:		printf("70 ohm");	break;
	case 3:		printf("50 ohm");	break;
	default :	printf("Reserved");	break;
	}
	printf(", DRAM = ");
	switch(RAMODT) {
	case 0:		printf("No Odt\n"); 	break;
	case 1:		printf("75 ohm\n"); 	break;
	case 2:		printf("150 ohm\n");	break;
	case 3:		printf("50 ohm\n");	break;
...
#define NX_CLKPWR_RESETSTATUS	(0xC000F058)

#define	MMU_ON					(0)
#define	NONCACHE_IO_OFFSET		(0)	// refer to pts.h

void cpu_cfg_info(void)
{
	unsigned int clkmode[2], pllreg[2], resetstat;
	...
	switch (resetstat) {
		...
		case (1<<2): printf("[WATCHDOG]\n");		break;
		case (1<<3): printf("[SOFTWARE]\n");		break;
		default:
			printf("[Unknown]\n");
			break;
	}

	DBGOUT("%s: %s Frequency Configuration : \n", CFG_SYS_CPU_NAME, (CFG_SYS_CLKPWR_UPDATE==CTRUE?"DONE":"PASS"));
#if (0)
	DBGOUT( "\n" );
