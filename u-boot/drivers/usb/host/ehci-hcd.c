		 ...
		 int length, struct devrequest *req)
{
	...
	case USB_REQ_GET_STATUS | ((USB_DIR_IN | USB_RT_HUB) << 8):
		debug("USB_REQ_GET_STATUS: USB_DIR_IN | USB_RT_HUB\n");
	...
	case USB_REQ_SET_FEATURE | ((USB_DIR_OUT | USB_RT_PORT) << 8):
		debug("USB_REQ_SET_FEATURE: USB_DIR_OUT | USB_RT_PORT\n");
		reg = ehci_readl(status_reg);
		reg &= ~EHCI_PS_CLEAR;
		...
		case USB_PORT_FEAT_POWER:
			if (HCS_PPC(ehci_readl(&hccr->cr_hcsparams))) {
				reg |= EHCI_PS_PP;
				printf("====> enable port power\n");
				ehci_writel(status_reg, reg);
			}
			break;
		case USB_PORT_FEAT_RESET:
		debug("USB_PORT_FEAT_RESET:\n");
...
int usb_lowlevel_init(void)
{
	...
	// psw0523 for monitoring port change
	printf("After RUN Port Status(%p): 0x%x\n", &hcor->or_portsc[0], ehci_readl(&hcor->or_portsc[0]));
