	...
	*maf_id = chip->read_byte(mtd);
    ...
    if (!type->pagesize) {
        ...
        if (id_data[0] == id_data[6] && id_data[1] == id_data[7] &&
			...
			case 1:
				mtd->oobsize = 128;
				/* add by jhkim for K9GAG08F */
				if (id_data[3] & 1<<6)
					mtd->oobsize = 512;
				break;
			case 2:
				mtd->oobsize = 218;
				/* add by jhkim for K9GBG08U0A */
				if (id_data[3] & 1<<6)
					mtd->oobsize = 640;
				break;
			case 3:
	...
	if (!type->pagesize) {
		...
		/* Calc pagesize */
        if (tmp_id == 0xD5)
    		mtd->writesize = 1024 << ((extid & 0x3) + 1);
        else
		...
		/* Calc oobsize */
        if (tmp_id == 0xD5) {
            int tmp_oobid = (extid & 0x03) + ((extid & 0x10) >> 2);
            switch (tmp_oobid) {
            case 1: mtd->oobsize = 128; break;
            case 2: mtd->oobsize = 218; break;
            case 3: mtd->oobsize = 400; break;
            case 4: mtd->oobsize = 436; break;
            default:
		        printk(KERN_WARNING "%s: not supported oobid(0x%x)! \n",
                                            __func__, tmp_oobid);
                break;
            }
        }
		...
		extid >>= 2;
		/* Calc blocksize. Blocksize is multiples of 64KiB or 128KiB */
        if (tmp_id == 0xD5) {
		...
		printk(KERN_WARNING "\n%s: writesize=%d oobsize=%d erasesize=%d busw=%d\n",
 ...
 */
extern int nand_dev_check(struct mtd_info *mtd);	/* add by jhkim */
