...
//	if (!need_skip && !(flags & WITH_DROP_FFS)) {
	if (!need_skip && !(flags & WITH_DROP_FFS) && !(flags & WITH_YAFFS_OOB)) {	/* mod by jhkim for yaffs2 */
		rval = nand_write (nand, offset, (size_t *)length, buffer);

		/*
		 * add by jhkim for skip bad and continue
		 */
#ifdef CONFIG_MTD_NAND_VERIFY_WRITE_SKIP_BAD
		if (-EBADMSG == rval) {
			struct nand_chip *chip = nand->priv;
			loff_t bad_offset = (offset + *length) & ~(nand->erasesize - 1);
			size_t bad_write  = (offset + *length) &  (nand->erasesize - 1);

			printf ("Mark bad block 0x%08llx, written 0x%x, bad write 0x%x, rest 0x%x\n",
				bad_offset, *length, bad_write, left_to_write - (*length - bad_write));

			chip->block_markbad(nand, bad_offset);

			left_to_write -= (*length - bad_write);
			buffer  = p_buffer + (*length - bad_write);
			*length = left_to_write;
			offset  = bad_offset + blocksize;

			/* write to next block */
			rval = nand_write_skip_bad(nand, offset, length, buffer, flags);
		}
#endif
		if (rval == 0)
			return 0;
...
__next_block:
		...
		if (flags & WITH_YAFFS_OOB) {
			...
			for (page = 0; page < pages; page++) {
				...
				if (-EBADMSG == rval) {
					struct nand_chip *chip = nand->priv;
					chip->block_markbad(nand, offset & ~(nand->erasesize - 1));
					printf ("Mark bad block 0x%08llx\n",
						offset & ~(nand->erasesize - 1));
					goto __next_block;
				}
#endif
			//	if (!rval)
				if (rval)	/* mod by jhkim */
					break;
