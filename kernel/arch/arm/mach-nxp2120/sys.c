 ...
 *
 * 	usage	: #define	__NR_syscall  223
 *			: user apps syscall(__NR_syscall, start, end, opt)
 *  opt 	: 0	= cache flush all
 *			: 1 = dcache flush area (user addr)
 *			:
 */
asmlinkage long
...
{
	u_long flags;
	DBGOUT("%s: sa=0x%08x, ea=0x%08x, opt=%d)\n", __func__, start, end, opt);

	local_irq_save(flags);

	switch (opt) {
	case 0:	flush_cache_all();
			break;
#if defined(CONFIG_ANDROID)			
	case 1:	flush_cache_user_range(start, end);	// kernel clean_dcache_area
			break;
#endif			
	default:
			break;
