...
static void gpio_irq_handler(unsigned int irqno, struct irq_desc *desc)
{
	...
	for (mod = 0; NX_GPIO_GetNumberOfModule() > mod; mod++)	{
		...
		irq  = (bit + IRQ_GPIO_START + (mod<<5));
		desc = irq_desc + irq;

		if (desc && desc->action)	{

			flags  = desc->action->flags;
			detmod = NX_GPIO_GetInterruptMode(mod, bit);

			#if (0)
			/* disable irq reentrant when level detect */
			if (!(detmod & 0x2)) desc->action->flags |= IRQF_DISABLED;
			#else
			...
			desc->action->flags = flags;
		} else {
			printk(KERN_ERR "Fail, not registered gpio irq:%d, grp:%d, bit:%d \n",
				irqno, mod, bit);
		}
...
static void __init alive_init_irq(void)
{
	...
	for (i = 0; 4 > i; i++) {
		for (d = 0; 6 > d; d++) {
			NX_ALIVE_SetDetectMode(d, i, CFALSE);
		}   
