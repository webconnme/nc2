...
#endif
{
	...
	/* source timer run */
	NX_TIMER_SetTimerCounter(ch, __s_counter);
...
{
	int ch = src_timer.ch;
	__s_counter = NX_TIMER_GetTimerCounter(ch);
	return __s_counter;
}
...
static void clock_event_shutdown(void)
{
	...
	if (CTRUE == NX_TIMER_GetClockDivisorEnable(ch)) {
		NX_TIMER_Stop(ch);
		NX_TIMER_ClearInterruptPendingAll(ch);
		NX_TIMER_GetInterruptPendingAll(ch);
	}
}
...
{
	int    ch 	 = evt_timer.ch;
	int	  irq    = evt_timer.irq;
	u_int period = evt_timer.period;

	DBGOUT("+ %s (ch:%d, mode:0x%x, period:%d, irq:%s)\n",
		__func__, ch, mode, period, irqs_disabled()?"OFF":"ON");
	...
	if (CLOCK_EVT_MODE_RESUME == mode) {
		volatile int count = 100;
		while ( CFALSE == NX_INTC_GetInterruptPending(irq) &&
				count-- >= 0) {
		...
		if (0 > count)
			lldebugout("%s: Fail timer(%d) run, system hanged\n", __func__, ch);
	}
	DBGOUT("- %s (ch:%d)\n", __func__, ch);
}
