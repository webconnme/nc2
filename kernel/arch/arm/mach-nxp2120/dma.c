...
int soc_dma_transfer(struct dma_trans *tr)
{
	...
	if (DMA_MODE_BUFFER == dma_stat[ch].mode) {
		...
		while (wait-- > 0 &&
			...
			printk(KERN_ERR "%d: EROR, command buffer ready \n", wait);
			udelay(1);
		}
	...
	case DMA_TRANS_MEM2IO:
		NX_DMA_TransferMemToIO(ch, (const void*)tr->mio.srcbase, tr->mio.dstbase,
					tr->mio.dst_id, tr->mio.dst_bit, tr->mio.length);
		break;
	case DMA_TRANS_IO2MEM:
		NX_DMA_TransferIOToMem(ch, tr->iom.srcbase, tr->iom.src_id, tr->iom.src_bit,
					(void*)tr->iom.dstbase, tr->iom.length);
		break;
	case DMA_TRANS_MEM2MEM:
		NX_DMA_TransferMemToMem(ch, (const void*)tr->mm.srcbase, (void*)tr->mm.dstbase,
					tr->mm.length);
		break;
	case DMA_TRANS_MEM2SIO:
		NX_DMA_TransferMemToSIO(ch, (const void*)tr->mio.srcbase, tr->mio.dstbase,
					tr->mio.dst_id, tr->mio.dst_bit, tr->mio.length);
		break;
	case DMA_TRANS_SIO2MEM:
		NX_DMA_TransferSIOToMem(ch, tr->iom.srcbase, tr->iom.src_id, tr->iom.src_bit,
					(void*)tr->iom.dstbase, tr->iom.length);
		break;
	default:
		ret = -2;
...
/*------------------------------------------------------------------------------
 * 	Description	: DMA irq enable/disable
 *	In[tr]		: requested DMA handle
 *	In[enb]		: 0 = disable irq
 *				: 1 = enable irq
 *	Return 		: 0 = success.
 *				:-1 = unknown DMA handle
 *				:-2 = not support DMA transfer mode
...
int soc_dma_irq_enable(struct dma_trans *tr, int enable)
{
	...
	/*
	 * when command buffer, Next command buffer irq mode
	 */
	NX_DMA_SetInterruptEnableAll(ch, enable?CTRUE:CFALSE);
