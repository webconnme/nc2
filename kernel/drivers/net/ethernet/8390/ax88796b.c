...
static void ax88796b_watchdog (unsigned long arg)
{
	...
	// [FALINUX]
	if( 0x1 == readb (ax_local->membase+EN0_ISR) )
	{
		spin_lock_irqsave (&ax_local->page_lock, flags);
		writeb (E8390_PAGE0 | E8390_NODMA, ax_local->membase + E8390_CMD);
		writeb (0x00, ax_local->membase + EN0_IMR);
		writeb (ENISR_ALL, ax_local->membase + EN0_IMR);
		spin_unlock_irqrestore (&ax_local->page_lock, flags);		
		if( 0x1 == readb (ax_local->membase+EN0_ISR) ) {
			printk("* EN0_ISR 0x01\n");
		}	
...
//[FA]	writeb (E8390_NODMA | E8390_PAGE0, ax_base + E8390_CMD);
//[FA]	writeb (0x00, ax_base + EN0_IMR);
...
//[FA]	writeb (ENISR_ALL, ax_base + EN0_IMR);
...
static int ax88796b_open (struct net_device *ndev)
{
	...
	ret = request_irq (ndev->irq, &ax_interrupt, IRQF_SHARED | IRQF_TRIGGER_LOW, ndev->name, ndev);		// IRQF_SHARED | IRQF_TRIGGER_LOW
	if (ret) {
		PRINTK (ERROR_MSG, PFX " unable to get IRQ %d (errno=%d).\n", ndev->irq, ret);
		return -ENXIO;
	}
...
//	.ndo_set_multicast_list = ax88796b_set_multicast_list,
	.ndo_set_rx_mode	= ax88796b_set_multicast_list, // FALINUX
...
static int ax88796b_exit_module(struct platform_device *pdev) // FALINUX
{
...
//	.remove  = __devexit_p(ax88796b_exit_module),
	.remove  = ax88796b_exit_module, // FALINUX
